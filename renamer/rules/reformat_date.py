...
class ReformatDateRule(RenameRule):
    ...
    def _build_ui(self, layout: QVBoxLayout):
        layout.addWidget(QLabel("Pattern regex per trovare la data:"))
        self._find = QLineEdit(); self._find.setText(r"\d{4}-\d{2}-\d{2}")
        layout.addWidget(self._find)

        layout.addWidget(QLabel("Formato output (Python strftime):"))
        self._fmt = QLineEdit(); self._fmt.setText("%Y%m%d")
        layout.addWidget(self._fmt)

        layout.addWidget(QLabel(
            "Esempi formato: %Y=anno, %m=mese, %d=giorno, %H=ora, %M=minuti, %S=secondi"))
        layout.addWidget(QLabel(
            "Input supportati: YYYY-MM-DD, DD-MM-YYYY, MM-DD-YYYY, YYYYMMDD, DDMMYYYY, YYYY.MM.DD, DD.MM.YYYY"))
